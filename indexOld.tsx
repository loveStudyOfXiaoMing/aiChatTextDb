...
import { ConfigProvider, theme, Select, Button, Segmented, Modal, Input, Checkbox, Divider, Space, message, Tabs, Table as AntTable } from 'antd';
...
const CHAT_STORAGE_KEY = 'nova_chat_history_v1';
...
type TableEditorModalProps = {
    ...
    accent: { bg: string; text: string; bgSoft: string; shadow?: string };
    extraInfo?: string;
    isDark: boolean;
    aiLoading: boolean;
    aiPrompt?: string;
    ...
    return (
        ...
        >
            ...
            <Space direction="vertical" style={{ width: '100%' }} size="middle">
                {onChangeAiPrompt && (
                    <div className={`rounded-xl border p-3 flex gap-3 items-start ${isDark ? 'border-white/10 bg-[#0f172a]/60' : 'border-gray-200 bg-gray-50'}`}>
                        <div className={`p-2 rounded-lg ${accent.bg} text-white shadow ${accent.shadow || ''}`}>
                            <Sparkles size={16} />
                        </div>
                        <div className="flex-1 space-y-2">
                            <div className="flex items-center justify-between">
                                <div className="text-sm font-medium">AI 需求描述</div>
                                {onAiGenerate && (
                                    <Button
                                        type="primary"
                    ...
                    <div className="space-y-2">
                        {columns.map((col, idx) => (
                            <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                                ...
                                <Input value={col.comment} onChange={e => updateColumn(idx, { comment: e.target.value })} className="col-span-1" />
...
const App = () => {
    ...
    const [sqlTabs, setSqlTabs] = useState<SqlTab[]>([{ id: 'sql-1', title: 'SQL 1', content: '' }]);
    const [activeSqlTabId, setActiveSqlTabId] = useState('sql-1');
    const sqlTabCounter = useRef(1);
    ...
    const dbAiInputRef = useRef<HTMLTextAreaElement | HTMLInputElement>(null);
    const sqlInputRef = useRef<HTMLTextAreaElement>(null);
    const chatInputRef = useRef<HTMLTextAreaElement>(null);
    const [isTestingConn, setIsTestingConn] = useState(false);
    const [isLoadingSchema, setIsLoadingSchema] = useState(false);
    const [isAiDesigning, setIsAiDesigning] = useState<{ target: 'create' | 'design' | null }>({ target: null });
    ...
    const activeSqlTab = useMemo(() => {
        if (!sqlTabs.length) return null;
        return sqlTabs.find(tab => tab.id === activeSqlTabId) || sqlTabs[0];
    }, [sqlTabs, activeSqlTabId]);

    const updateActiveSql = (text: string, targetId?: string) => {
        const id = targetId || activeSqlTab?.id || activeSqlTabId;
        setSqlTabs(prev => prev.map(tab => tab.id === id ? { ...tab, content: text } : tab));
    };

    const addSqlTab = () => {
        const next = sqlTabCounter.current + 1;
        sqlTabCounter.current = next;
        const newId = `sql-${next}`;
        const newTab: SqlTab = { id: newId, title: `SQL ${next}`, content: '' };
    ...
    const closeSqlTab = (id: string) => {
        setSqlTabs(prev => {
            ...
            const fallback = filtered.find(tab => tab.id === activeSqlTabId) ? activeSqlTabId : filtered[filtered.length - 1]?.id;
            if (fallback && fallback !== activeSqlTabId) setActiveSqlTabId(fallback);
            return filtered;
        });
    };

    const handleSaveSqlFile = () => {
        const sqlText = activeSqlTab?.content?.trim();
        if (!sqlText) {
            showToast('没有可保存的 SQL', 'error');
            return;
        }
        const blob = new Blob([sqlText], { type: 'text/sql;charset=utf-8' });
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        link.download = `${(activeSqlTab?.title || 'query').replace(/\s+/g, '_')}.sql`;
        link.click();
        URL.revokeObjectURL(link.href);
        showToast('已保存 SQL 文件', 'success');
    };
    ...
    useEffect(() => {
        ...
        const storedChats = loadChatHistory();
    ...
    const runQuery = async (sqlOverride?: string, dbOverride?: string) => {
        try {
            const sqlValue = typeof sqlOverride === 'string'
                ? sqlOverride
                : (activeSqlTab?.content || '');
            if (typeof sqlOverride === 'string') {
                updateActiveSql(sqlOverride);
            }
    ...
    const saveChatHistory = (history: ChatMessage[]) => {
        try {
            const trimmed = history.slice(-200); // avoid unlimited growth
...
示例 JSON: {"type":"bar","xField":"字段A","yFields":["字段B"]}`;
    ...
    const handleChatSend = async () => {
        ...
        try {
            const systemPrompt = 'You are a helpful, knowledgeable database expert and AI assistant. 请用简洁中文回答，必要时给出 SQL 示例。';
            let aiText = '';

            if (aiConfig.provider === 'google') {
                if (!ai) {
                    showToast('请在设置中填写 Gemini API Key 或切换模型提供者', 'error');
                    setIsChatThinking(false);
                    return;
                }
                const historyText = historyWithUser.map(m => `${m.role === 'user' ? '用户' : '助手'}：${m.content}`).join('\n');
                ...
                aiText = result.text || "抱歉，我无法回答。";
            } else {
                const base = getAiBaseUrl() || 'https://api.openai.com/v1';
                if (!aiConfig.apiKey && aiConfig.provider !== 'ollama') {
                    showToast('请填写 API Key 或切换到 Ollama', 'error');
                    setIsChatThinking(false);
                    return;
                }
                const resp = await fetch(`${base}/chat/completions`, {
                    ...
                    body: JSON.stringify({
                        ...
                        messages: [
                            ...
                            ...historyWithUser.map(m => ({ role: m.role === 'user' ? 'user' : 'assistant', content: m.content }))
                        ]
                    })
                });
                const data = await resp.json();
                aiText = data?.choices?.[0]?.message?.content || "抱歉，我无法回答。";
            }

            const aiMsg: ChatMessage = { id: (Date.now() + 1).toString(), role: 'ai', content: aiText, timestamp: Date.now() };
    ...
    const handleSaveConnection = async () => {
        if (editingConnectionId) {
            const existing = findConnectionById(editingConnectionId);
            const prevRuntimeId = existing?.runtimeId;
            if (prevRuntimeId && isDesktop && window.desktopAPI?.close) {
                try {
                    await window.desktopAPI.close({ connId: prevRuntimeId });
                } catch (err) {
                    console.warn('close previous connection failed', err);
                }
            }
    ...
    const generateTableColumnsWithAi = async (target: 'create' | 'design') => {
        const tableName = target === 'create' ? newTableForm.name || 'new_table' : designTableData.name;
        ...
        try {
            ...
            const hint = tableName ? `优先使用提供的表名: ${tableName}。` : '';
            const instruction = `${baseInstruction} ${hint} ${idea ? `额外需求：${idea}` : ''}`;
            ...
            if (aiConfig.provider === 'google') {
                ...
                const resp = await fetch(`${base}/chat/completions`, {
                    ...
                    body: JSON.stringify({
                        ...
                        temperature: aiConfig.temperature,
                        messages: [
                            { role: 'system', content: 'Return JSON array of column definitions.' },
                            { role: 'user', content: instruction }
                        ]
                    })
                });
            ...
            if (!cols.length) {
                showToast('未生成字段，请重试或手动填写', 'error');
                return;
            }

            if (target === 'create') {
                setNewTableForm(f => ({ ...f, name: parsed.tableName || f.name || 'new_table', columns: cols }));
            } else {
                setDesignTableData(p => ({ ...p, name: parsed.tableName || p.name, columns: cols }));
            }
            showToast('AI 已生成字段', 'success');
        } catch (e: any) {
            showToast(`生成字段失败: ${e?.message || e}`, 'error');
        } finally {
            setIsAiDesigning({ target: null });
        }
    };
    ...
    const renderSidebar = () => {
        if (activeView === 'chat') {
            const recentPrompts = chatHistory.filter(m => m.role === 'user').slice(-20).reverse();
            return (
        ...
        `}>
                    <div className="p-5 border-b border-transparent">
                        ...
                        <div className="text-xs opacity-50 mt-1">自动保存最近 20 条提问</div>
                    </div>
                    <div className="p-3 space-y-2 overflow-y-auto flex-1 custom-scrollbar">
                        {recentPrompts.length === 0 && (
                            <div className={`p-3 rounded-lg text-xs opacity-60 border text-center ${isDark ? 'border-white/10 text-slate-300' : 'border-gray-200 text-slate-600'}`}>
                                暂无历史，开始对话吧
                            </div>
                        )}
                        {recentPrompts.map(msg => (
                            <button
                                key={msg.id}
                                onClick={() => { setChatInput(msg.content); chatInputRef.current?.focus(); }}
                                className={`w-full text-left p-3 rounded-lg flex items-start gap-2 text-sm transition-colors
                  ${isDark ? 'hover:bg-white/5 text-slate-200' : 'hover:bg-gray-100 text-slate-800'}
                `}
                            >
                                <MessageSquare size={14} className={colors.text} />
                                <div className="flex-1 min-w-0">
                                    <div className="truncate">{msg.content}</div>
                                    <div className="text-[10px] opacity-50 mt-1">{new Date(msg.timestamp).toLocaleString()}</div>
                                </div>
                            </button>
                        ))}
                    </div>
    ...
    const renderDatabaseView = () => {
        const dataSource = (results?.rows || []).map((row, idx) => ({
            key: idx.toString(),
            __index: idx + 1,
            ...row
        }));
        const tableColumns = [
            ...
            ...(results?.headers || []).map((h) => ({
                ...
                ellipsis: true,
                render: (val: any) => {
                    if (val === null || val === undefined) return <span className="opacity-50">NULL</span>;
        ...
        return (
            <div className="flex-1 flex flex-col h-full relative min-w-0" style={{ pointerEvents: 'auto' }}>
                <div className="flex-1 flex flex-col px-6 pb-6 pt-8 gap-4 overflow-hidden">
                ...
                ${editorCollapsed ? 'h-12' : 'h-[38%] min-h-[220px]'}
              `}>
                                <div className={`flex items-center justify-between px-4 py-2.5 border-b select-none flex-wrap gap-3 ${isDark ? 'border-[#1e293b] bg-[#1e293b]/30' : 'border-gray-100 bg-gray-50'}`}>
                                    <div className="flex items-center gap-2 text-xs font-mono opacity-70">
                                        <Terminal size={14}/> SQL 编辑器
                                    </div>
                                    <div className="flex items-center gap-2 flex-wrap">
                                        <Button size="small" icon={<Plus size={14} />} onClick={addSqlTab}>新建标签</Button>
                                        <Button size="small" icon={<Save size={14} />} onClick={handleSaveSqlFile}>保存 SQL</Button>
                                        <Button type="primary" size="small" icon={<Play size={14} fill="currentColor" />} onClick={() => runQuery()} className={colors.bg}>
                                            运行
                                        </Button>
                                ...
                                {!editorCollapsed && (
                                    <>
                                        <div className={`px-4 pt-2 ${isDark ? 'bg-[#0f1218]' : 'bg-white'}`}>
                                            <Tabs
                                                size="small"
                                                type="card"
                                                activeKey={activeSqlTab?.id}
                                        ...
                                        <div className="flex-1 relative group">
                                            <div className={`absolute inset-y-0 left-0 w-12 border-r flex flex-col items-end pr-3 pt-4 font-mono text-xs opacity-20 select-none ${isDark ? 'border-[#1e293b]' : 'border-gray-100'}`}>
                                                {Array.from({ length: 14 }).map((_, idx) => <div key={idx}>{idx + 1}</div>)}
                                            </div>
                                            <textarea
                                                ref={sqlInputRef}
              ...
              `}>
                                ...
                                <div className="flex-1 overflow-auto custom-scrollbar relative">
                                    {activeTab === 'results' ? (
                                        results?.error ? (
                                            ...
                                            <div className="flex flex-col items-center justify-center h-full text-sm opacity-50 gap-2">
                                                <LayoutGrid size={20} />
                                                <span>暂无查询结果，执行 SQL 后查看</span>
                                            </div>
                                        )
                                    ) : activeTab === 'chart' ? (
                                ...
                                {activeTab === 'results' && (
                                    <div className={`h-9 border-t flex items-center justify-between px-4 text-xs select-none ${isDark ? 'border-[#1e293b] bg-[#0f1218]' : 'border-gray-200 bg-gray-50'} opacity-70`}>
                                        ...
                                        <div className="text-xs opacity-70">当前标签: {activeSqlTab?.title || 'SQL'}</div>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="w-[360px] flex flex-col gap-3">
                            <div className={`rounded-2xl border shadow-xl h-full min-h-[360px] overflow-hidden ${isDark ? 'bg-[#0f1218] border-[#1e293b]' : 'bg-white border-gray-200'}`}>
                                <div className={`px-4 py-3 flex items-center justify-between border-b ${isDark ? 'border-[#1e293b]' : 'border-gray-100'}`}>
                                    <div className="flex items-center gap-3">
                                        <div className={`p-2 rounded-xl ${colors.bg} text-white shadow-lg shadow-black/10`}>
                                            <Sparkles size={16} />
                                        </div>
                                        <div>
                                            <div className="text-sm font-semibold">AI 指令对话框</div>
                                            <div className="text-xs opacity-60">描述你想要的 SQL/分析</div>
                                        </div>
                                    </div>
                                    <Button size="small" icon={<RefreshCcw size={14} />} onClick={() => setDbAiPrompt('')}>清空</Button>
                                </div>
                                <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                                    <div className={`text-xs leading-relaxed ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
                                        向右侧的对话框输入指令，例如“统计本周新增用户”或“为订单表生成 5 条插入语句”。支持直接执行生成 SQL。
                                    </div>
                                    {insight && (
                                        <div className="flex gap-3 items-start">
                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${colors.bg} text-white`}><Bot size={14} /></div>
                                            <div className={`flex-1 rounded-2xl p-3 text-sm shadow-sm ${isDark ? 'bg-[#111827] border border-white/5 text-slate-100' : 'bg-gray-50 border border-gray-200 text-slate-700'}`}>
                                                <div className="text-[11px] uppercase opacity-50 mb-1">AI 洞察</div>
                                    ...
                                    {dbAiPrompt && (
                                        <div className="flex gap-3 items-start justify-end">
                                            <div className="flex-1 max-w-full">
                                                <div className={`${colors.bg} text-white rounded-2xl p-3 shadow-md`}>
                                                    <div className="text-[11px] uppercase opacity-80 mb-1">待发送</div>
                                                    <div className="whitespace-pre-wrap leading-relaxed">{dbAiPrompt}</div>
                                                </div>
                                            </div>
                                            <div className="w-8 h-8 rounded-full bg-slate-700 text-slate-200 flex items-center justify-center mt-1">
                                                <User size={14} />
                                            </div>
                                        </div>
                                    )}
                                </div>
                                <div className={`border-t px-4 py-3 ${isDark ? 'border-[#1e293b] bg-[#0b111d]' : 'border-gray-100 bg-gray-50'}`}>
                                    <div className={`rounded-2xl border p-3 shadow-inner ${isDark ? 'bg-[#0f172a] border-white/10' : 'bg-white border-gray-200'}`}>
                    <textarea
                        ref={dbAiInputRef as React.RefObject<HTMLTextAreaElement>}
                        value={dbAiPrompt}
                        ...
                        placeholder="像聊天一样输入数据库指令，Enter 执行，Shift+Enter 换行"
                        className={`w-full bg-transparent border-none outline-none resize-none text-sm ${isDark ? 'text-slate-100' : 'text-slate-800'}`}
                        rows={3}
                    />
                                    </div>
                                    <div className="flex justify-between items-center mt-3">
                                        <span className="text-xs opacity-50">支持描述建表、生成 SQL、数据洞察</span>
                                        <Space>
                                            <Button size="small" onClick={() => setDbAiPrompt('')}>重置</Button>
                                            <Button type="primary" size="small" icon={isDbAiThinking ? <Loader2 size={14} className="animate-spin" /> : <Wand2 size={14} />} loading={isDbAiThinking} onClick={handleDbAskAi} disabled={!dbAiPrompt.trim()}>
                                                执行
                                            </Button>
                                        </Space>
                                    </div>
          ...
          `}>
                ...
                {modals.newConn && (
                    <Modal
                        ...
                        footer={
                            <div className="flex justify-between items-center">
                                <Button onClick={handleTestConnection} loading={isTestingConn}>测试连接</Button>
                                <div className="flex gap-2">
                                    <Button onClick={closeConnectionModal}>取消</Button>
                                    <Button type="primary" onClick={handleSaveConnection}>{editingConnectionId ? '保存并重连' : '创建并连接'}</Button>
                                </div>
                            </div>
                        }
                        maskClosable={false}
                    ...
                    >
                        <div className="space-y-4">
                            <div className="space-y-2">
                                <div className="text-xs opacity-70">数据库类型</div>
                                <Segmented
                                    block
                                    value={connectionForm.type}
                                    onChange={(val) => setConnectionForm(f => ({ ...f, type: val as DbType, port: DEFAULT_PORT[val as DbType] }))}
                                    options={DB_OPTIONS.map(opt => ({ label: opt.name, value: opt.type }))}
                                />
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div className="col-span-2 space-y-1">
                                    <div className="text-xs opacity-70">连接名称</div>
                                    <Input
                                ...
                                <div className="space-y-1">
                                    <div className="text-xs opacity-70">主机 / IP</div>
                                    <Input
                                ...
                                <div className="space-y-1">
                                    <div className="text-xs opacity-70">端口</div>
                                    <Input
                                        type="number"
                                ...
                                <div className="space-y-1">
                                    <div className="text-xs opacity-70">用户名</div>
                                    <Input
                                        placeholder="如 root"
                                ...
                                <div className="space-y-1">
                                    <div className="text-xs opacity-70">密码</div>
                                    <Input.Password
                                        placeholder="数据库密码"
                            ...
                            <div className="text-xs opacity-70">保存后会自动重连并刷新库树；无需填写数据库名，会自动列出。</div>
                        </div>
                    </Modal>
                )}
                ...
                <TableEditorModal
                    ...
                    isDark={isDark}
                    aiLoading={isAiDesigning.target === 'create'}
                ...
                <TableEditorModal
                    ...
                    isDark={isDark}
                    aiLoading={isAiDesigning.target === 'design'}
